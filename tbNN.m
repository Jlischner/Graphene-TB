...
for ii = 1:N/2;
  ...
  for jj = 1:length(C1b);
    ...
    if(abs( norm(C1ii-C1b(jj,:)) - aNN) < tol);

      if( norm(indx(jj,:)) > tol);
	H(ii,partner(jj)) = t*exp(2*pi*I*(kx*indx(jj,1) + ky*indx(jj,2) ));
	entries = [entries; ii partner(jj) t indx(jj,1) indx(jj,2)];
      else;
	H(ii,partner(jj)) = t;
	entries = [entries; ii partner(jj) t indx(jj,1) indx(jj,2)];
      endif;
  ...
  C2ii = C2s(ii,:);
  for jj = 1:length(C2b);
    if(abs( norm(C2ii-C2b(jj,:)) - aNN) < tol);

      if(norm(indx(jj,:)) > tol);
	H(ii+N/2,partner(jj)+N/2) = t*exp(2*pi*I*(kx*indx(jj,1) + ky*indx(jj,2) ) );
	entries = [entries; ii+N/2 partner(jj)+N/2 t indx(jj,1) indx(jj,2)];
      else;
	H(ii+N/2,partner(jj)+N/2) = t;
	entries = [entries; ii+N/2 partner(jj)+N/2 t indx(jj,1) indx(jj,2)];
      endif;
    endif;
